...
| 25  | **Any placed control**   | A layout guesses each item's box, so content wider than the guess overflows it and, on a ring, overlaps a neighbour                                   |
| 25  | **Any placed control**   | Nothing stops a layout outgrowing the room it is in — a ring can reach past the screen, and a placed toolbar has nothing to collapse into             |
| 25  | **`Stepper`, placed**    | `renderBody` is dropped without a word, a curve having nowhere to put a panel                                                                         |
...
| 25  | **`Tree`, placed**  | No connector slot, so a radial tree's structure reads only from where its nodes sit                                                  |
...
| 24  | **Per-sample defs, and knobs that follow the key** | A sample's tuned numbers are unreachable module constants, in three registries — gradients, Placement layouts and scanline keyframes. Settled: an entry is a family plus its own options bag, knobs are derived from a per-family descriptor, the descriptor is its own module beside the family, a registry converts in one pass rather than growing a parallel table, the shared descriptor type sits at the `Samples/` root, a preset stops existing once a knob reaches it, and the gradients collapse only their suffixes in the first pass with `fill` and `elastic` exempt. Every registry converts, `Placement/Layouts` first as the proof of concept. Open: what a turned knob means for the key.            |
| 25  | **Arbitrary placement across controls**            | Items placeable anywhere rather than along a line. Built: the `Placement` abstract with the placed box, the picking, the sector and link path builders, one home for every layout sample, and ten controls taking an optional layout — `Menu` with `WheelMenu` and `FanMenu` over it, `OverheadWheel`, `Paginator`, `RadioGroup`, `Tabs`, `Stepper`, `Toolbar`, `Sortable`, `Tree` and `Formation`, the hold-and-flick gesture with its click-open and keyboard routes, and the split of the one arc factory into a band of wedges that nests and an arc of boxes given its own width and height, spaced evenly along the curve. Left: the faults the proving pass found, and a travel threshold nobody has measured. |
