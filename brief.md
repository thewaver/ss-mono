...
| 26  | **Arbitrary placement across controls**             | Items placeable anywhere rather than along a line. Built: the `Placement` abstract, `Menu`'s optional layout, the hemisphere and fan, `WheelMenu` (full and half turn, per-item arcs, concentric submenus aimed at their opener, a close control that is an ordinary item), `FanMenu` (levels that replace rather than stack, with the opener as the way back), a `Placement` page, and `OverheadWheel` drawing from the same layout. Left: the hold-and-flick gesture and the proving pass against `Paginator`. |
