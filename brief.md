...
| [Planned projects](#planned-projects)                   |     6 |
...
| 9   | **`ColorInput`**    | No native color field underneath, so a plain form submit carries no value and there is no way back to the OS picker                  |
...
| 24  | **`Tree`, placed**  | No connector slot, so a radial tree's structure reads only from where its nodes sit                                                  |
...
| 24  | **Arbitrary placement across controls**       | Items placeable anywhere rather than along a line. Built: the `Placement` abstract with the placed box, the picking, the sector and link path builders, one home for every layout sample, and ten controls taking an optional layout — `Menu` with `WheelMenu` and `FanMenu` over it, `OverheadWheel`, `Paginator`, `RadioGroup`, `Tabs`, `Stepper`, `Toolbar`, `Sortable`, `Tree` and `Formation`, the hold-and-flick gesture with its click-open and keyboard routes, and the split of the one arc factory into a band of wedges and an arc of boxes spaced evenly along the curve, each stated in ratios rather than lengths. Left: the faults the proving pass found, and a travel threshold nobody has measured. |
| 25  | **Scanline page shape**                       | Twelve example boxes where eleven differ only by which compute function and which knobs. The knobs are derived already; collapsing the boxes into one example with a picker is what is left, and the readouts and the picker's shape need designing.                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 26  | **Gradient knobs, exposed wide**              | Every tracked sample states its own numbers now, five to fifteen knobs each, each defaulting to the constant it was tuned with. Deliberately over-exposed: the user narrows it by asking which to hide, the way `elastic_*` lost its step knob and the hand samples lost their sweep lead. The timed side is settled — `scan`, `sweep`, `fill` and `merge` keep their tuning inline because it defines the effect.                                                                                                                                                                                                                                                                                                                          |
