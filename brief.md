...
| 24  | **Any placed control**   | A layout guesses each item's box, so content wider than the guess overflows it and, on a ring, overlaps a neighbour                                   |
...
| 26  | **Gradient knobs, exposed wide**              | Every tracked sample states its own numbers now, five to fifteen knobs each, each defaulting to the constant it was tuned with. Deliberately over-exposed: the user narrows it by asking which to hide, the way `elastic_*` lost its step knob. The timed side is half done — `scan`, `sweep`, `fill` and `merge` still hold their tuning inline.                                                                                                                                                                                                                                                                                                                                                                     |
