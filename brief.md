...
| 26  | **Any placed control**   | A layout guesses each item's box, so content wider than the guess overflows it and, on a ring, overlaps a neighbour                                   |
| 26  | **Any placed control**   | Nothing stops a layout outgrowing the room it is in — a ring can reach past the screen, and a placed toolbar has nothing to collapse into             |
| 26  | **`Stepper`, placed**    | `renderBody` is dropped without a word, a curve having nowhere to put a panel                                                                         |
...
| 26  | **`Tree`, placed**  | No connector slot, so a radial tree's structure reads only from where its nodes sit                                                  |
