...
export namespace LabelUtils {
     ...
     *
     * An `aria-label` replaces the accessible name entirely, so a control inside a labeled field would
    ...
    export const resolveAriaLabel = (getAriaLabel?: () => string) => {
        ...
        if (labelContext.getIsLabeled() && getAriaLabel) {
            console.warn(
        ...
        return () => (labelContext.getIsLabeled() ? undefined : getAriaLabel?.());
    };
}
