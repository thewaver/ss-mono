...
describe("CellAnimationPlaybackUtils", () => {
    ...
    it("charges an alternating pass for both trips plus the hold between them", () => {
        expect(CellAnimationPlaybackUtils.computeCycleDurationMs(2000, { dir: "alternate" })).toBe(4000);
        expect(CellAnimationPlaybackUtils.computeCycleDurationMs(2000, { dir: "alternate", holdMs: 1000 })).toBe(5000);
        expect(
            CellAnimationPlaybackUtils.computeCycleDurationMs(2000, { dir: "alternate-reverse", holdMs: 1000 }),
        ).toBe(5000);
        expect(CellAnimationPlaybackUtils.computeCycleDurationMs(2000, { dir: "alternate", holdMs: -1000 })).toBe(4000);
    });
    ...
    it("spends the first half of an alternating pass going out and the second coming back", () => {
        const at = (timeline: number) =>
            CellAnimationPlaybackUtils.computeGlobalTimeline(timeline, 2000, { dir: "alternate" });
    ...
    it("parks at the far end for exactly the hold's share of the cycle", () => {
        const at = (timeline: number) =>
            CellAnimationPlaybackUtils.computeGlobalTimeline(timeline, 2000, { dir: "alternate", holdMs: 1000 });
    ...
    it("holds at the far end throughout rather than dividing by zero, when there is no trip to make", () => {
        expect(CellAnimationPlaybackUtils.computeGlobalTimeline(0, 0, { dir: "alternate", holdMs: 1000 })).toBe(1);
        expect(CellAnimationPlaybackUtils.computeGlobalTimeline(1, 0, { dir: "alternate", holdMs: 1000 })).toBe(1);
    });
});
