...
const createTrailStamp = (
    ...
    createEffect(() => {
        ...
        lastOrigin = origin;
        speed += ((step ?? NO_SPEED) - speed) * (opts?.smearSmoothing ?? SMEAR_SMOOTHING);
        ...
        setStamp({
            ...
            stretch: MathUtils.lerp(
                NO_STRETCH,
                opts?.smearMax ?? SMEAR_MAX,
                MathUtils.clamp01(speed / (opts?.smearFullStepRatio ?? SMEAR_FULL_STEP_RATIO)),
            ),
        });
    });
    ...
    return {
        ...
        getColors: (colors: SVGDefsColors) =>
            isCycling
                ? computePoolColors(getCycleColor(colors, getStamp()?.bornMs ?? 0, opts), getAlpha(), opts)
