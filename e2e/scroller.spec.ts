...
/**
 * The table of contents is not a `Scroller` — it sits on this page because the abstract under it,
 * `ElementObserverUtils.createViewportCurrentIndexObserver`, has no menu entry of its own. What it promises is
 * the relationship between two lists: the link marked `aria-current` is the one for the last heading scrolled
 * past a line near the top, and pressing a link takes the reader to its heading. So these read the mark off
 * the links and the position off the headings, and never ask what the mark looks like.
 *
...
test.describe("a table of contents following the page", () => {
    ...
    const MARKED = `${LINKS}[aria-current="true"]`;
    ...
    test("pressing a link brings its heading into view, focuses it, and marks that link alone", async ({ page }) => {
        ...
        for (const index of [2, count - 1, 0]) {
            ...
            await expect(links.nth(index), "that link is the current one").toHaveAttribute("aria-current", "true");
