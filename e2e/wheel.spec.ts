...
test.beforeEach(async ({ page }) => {
    ...
    await expect(page.locator(wheel(SIDEWAYS))).toBeVisible();
...
test("the wheel and every wedge say what they are, beyond what their roles convey", async ({ page }) => {
    await expect(page.locator(wheel(SIDEWAYS))).toHaveAttribute("role", "group");
    await expect(page.locator(wheel(SIDEWAYS))).toHaveAttribute("aria-label", "Prize drum, turning sideways");

    await expect(
        page.locator(wedge(SIDEWAYS)).first(),
...
test("the wheel renders no button, and the page's own is a real button with a real name", async ({ page }) => {
    await expect(page.locator(spin("sideways"))).toHaveAttribute("type", "button");
    await expect(page.locator(`${wheel(SIDEWAYS)} button`), "no wheel renders a button of its own").toHaveCount(0);
    await expect(page.locator(`${wheel(REEL)} button`)).toHaveCount(0);

    await expect(page.locator(spin("sideways")), "each page control sits outside its wheel and drives it").toHaveCount(
        1,
    );
    await expect(page.locator(spin("reel"))).toHaveCount(1);
});
...
test("spinning lands on a wedge and says which one", async ({ page }) => {
    ...
    await expect.poll(() => turnedAngle(page, SIDEWAYS), { timeout: SPIN_TOTAL_MS * 2 }).toBeGreaterThan(0);
...
test("a spin cannot be asked for twice, because the second request has nowhere to go", async ({ page }) => {
    ...
    await expect(
        page.locator(spin("sideways")),
        "the control says so rather than quietly ignoring the press",
    ).toHaveAttribute("aria-disabled", "true");

    await expect
        .poll(() => page.locator(spin("sideways")).getAttribute("aria-disabled"), { timeout: SPIN_TOTAL_MS * 2 })
        .toBe(null);
});

test("the wheel turns by itself while it waits to be spun", async ({ page }) => {
    const before = await transformOf(page, SIDEWAYS);

    await expect.poll(() => transformOf(page, SIDEWAYS), { timeout: IDLE_DELAY_MS * 3 }).not.toBe(before);
});
...
test("a spin buys the prize a rest, so the wheel stays on it long enough to be read", async ({ page }) => {
    ...
    const settled = await transformOf(page, SIDEWAYS);

    await page.waitForTimeout(IDLE_DELAY_MS * 2);

    expect(
        await transformOf(page, SIDEWAYS),
...
test("and the rest is only a rest, so the wheel picks up again once it has run out", async ({ page }) => {
    ...
    await expect
        .poll(() => transformOf(page, SIDEWAYS), {
...
test("a visitor who has asked for less motion gets a wheel that waits to be spun", async ({ page }) => {
    ...
    expect(await transformOf(page, SIDEWAYS), "nothing turns until it is asked to").toBe(before);
...
test("an idling wheel has picked nothing, and goes back to having picked nothing after a spin", async ({ page }) => {
    ...
    expect(await pickedCards(page, SIDEWAYS), "the turn is not a selection").toEqual([]);
 ...
 */
test("a drum picks out the face at its marker, and says which one it landed on", async ({ page }) => {
    await setField(page, "restDurationMs", String(MEDIUM_REST_MS));
...
test("a disabled wheel neither spins nor turns", async ({ page }) => {
    ...
    await expect(page.locator(spin("sideways"))).toHaveAttribute("aria-disabled", "true");

    const before = await transformOf(page, SIDEWAYS);

    await page.waitForTimeout(IDLE_DELAY_MS * 2);

    expect(await transformOf(page, SIDEWAYS)).toBe(before);
});
